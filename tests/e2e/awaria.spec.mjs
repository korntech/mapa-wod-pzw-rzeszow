...
import { CONFIG, KAFLE_HOST, KOMUNIKAT_ZAPASOWY, wczytaneKafle } from './pomocnicze.mjs';
...
test('podkład domyślny nie odpowiada → komunikat i podkład zapasowy', async ({ page }) => {
  ...
  const warstwy = await page.$$eval('img.leaflet-tile', (imgs) => [...new Set(imgs.map((i) => new URL(i.src).searchParams.get('LAYER')))]);
...
test('cache kafli (Worker) nie odpowiada → łowiska dalej widoczne i klikalne', async ({ page }) => {
  ...
  await page.route((url) => url.host === KAFLE_HOST, (route) => route.abort('connectionrefused'));
  await page.goto('./');
  await expect(page.locator('#list .item').first()).toBeVisible();
  await expect(page.getByText(KOMUNIKAT_ZAPASOWY)).toBeVisible({ timeout: 30_000 });
